...
const processAuthResponse = async (id, authData, redirect, user=null) => {
    ...
    } else if(authData.cookies && Object.keys(authData.cookies).length > 0) {
    ...
    if(existingAccount) {
        ...
        if(existingAccount.auth?.ent) user.auth.ent = existingAccount.auth.ent;
        if(existingAccount.auth?.cookies && (!user.auth.cookies || Object.keys(user.auth.cookies).length === 0)) {
            user.auth.cookies = existingAccount.auth.cookies;
        }
    }
...
export const redeemRedirectUrl = async (id, redirectUrl) => {
    let rateLimit = isRateLimited("auth.riotgames.com");
    if(rateLimit) return {success: false, rateLimit: rateLimit};

    const [rso] = extractTokensFromUri(redirectUrl);
    if(!rso) {
        return {success: false, error: "invalid_tokens"};
    }

    try {
        const user = await processAuthResponse(id, {cookies: {}}, redirectUrl);
        addUser(user);
        return {success: true, username: user.username, user};
    } catch(e) {
        console.error("Error redeeming redirect URL:", e);
