import {redeem2FACode, redeemCookies, redeemRedirectUrl, redeemUsernamePassword} from "./auth.js";
...
export const queueRedirectUrlLogin = async (id, redirectUrl) => {
    ...
    const c = queueCounter++;
    queue.push({
        operation: Operations.REDIRECT_URL,
        c, id, redirectUrl
    });
    console.log(`Added redirect URL login to auth queue for user ${id} (c=${c})`);
    ...
    try {
        switch (item.operation) {
            ...
            case Operations.COOKIES:
                ...
                break;
            case Operations.REDIRECT_URL:
                result = await redeemRedirectUrl(item.id, item.redirectUrl);
                break;
