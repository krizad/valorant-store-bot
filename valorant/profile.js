...
export const getAccountInfo = async (user, interaction) => {
    if (!config.HDevToken || config.HDevToken.trim() === "") {
        return {
            success: false,
            error: "HDevToken is not configured by the bot owner. Please set HDEV_TOKEN in .env or config.json (Get free key at https://api.henrikdev.xyz/dashboard/)"
        };
    }
...
export const fetchMatchHistory = async (interaction, user, mode="competitive") => {
    if (!config.HDevToken || config.HDevToken.trim() === "") {
        return {
            success: false,
            error: "HDevToken is not configured by the bot owner. Please set HDEV_TOKEN in .env or config.json"
        };
    }
