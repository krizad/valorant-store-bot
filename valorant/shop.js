...
import { addBundleData, addPrice, getSkin, getSkinFromSkinUuid } from "./cache.js";
...
export const getShop = async (id, account = null) => {
    ...
    // save individual skin offer prices from storefront
    if (json.SkinsPanelLayout?.SingleItemStoreOffers) {
        for (const offer of json.SkinsPanelLayout.SingleItemStoreOffers) {
            if (offer.OfferID && offer.Cost) {
                const cost = offer.Cost[Object.keys(offer.Cost)[0]];
                addPrice(offer.OfferID, cost);
            }
        }
    }
...
export const getBalance = async (id, account = null) => {
    ...
    if (!json.Balances) return { success: false, error: "Balances not found" };

    return {
        success: true,
        vp: json.Balances["85ad13f7-3d1b-5128-9eb2-7cd8ee0b5741"] || 0,
        rad: json.Balances["e59aa87c-4cbf-517a-5983-6e81511be9b7"] || 0,
        kc: json.Balances["85ca954a-41f2-ce94-9b45-8ca3dd39a00d"] || 0
    };
}
