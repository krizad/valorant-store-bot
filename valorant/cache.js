...
export const fetchData = async (types = null, checkVersion = false) => {
    try {
        ...
        const shouldFetchPrices = types.includes(prices) && (!prices || prices.version !== gameVersion || !prices.timestamp || (Date.now() - prices.timestamp > 24 * 60 * 60 * 1000));
        if (shouldFetchPrices) promises.push(getPrices(gameVersion));
...
const getPrices = async (gameVersion, id = null) => {
    if (!config.fetchSkinPrices) return false;

    if (!prices) prices = {};

    // if no ID is passed, try with available users
    if (id === null) {
        for (const userId of getUserList()) {
            const user = getUser(userId);
            if (!user?.auth) continue;

            const res = await getPrices(gameVersion, userId);
            // If the endpoint is permanently deprecated (404), stop trying other users
            if (res === 404 || res === true) break;
        }
    ...
    try {
        ...
        if (req.statusCode === 200) {
            ...
            console.log(`Riot store/v1/offers/ returned 404 (deprecated endpoint). Using cached and storefront prices.`);
            prices.version = gameVersion;
            prices.timestamp = Date.now();
            saveSkinsJSON();
            return 404;
        } else {
            console.log(`Riot store/v1/offers/ returned ${req.statusCode} (deprecated endpoint). Using cached and storefront prices.`);
        }
    } catch (e) {
        console.warn("Could not fetch skin prices from store/v1/offers:", e.message || e);
    }
...
export const addPrice = (uuid, price) => {
    if (!prices) prices = {};
    prices[uuid] = price;
    saveSkinsJSON();
}

export const addPrices = (priceMap) => {
    if (!prices) prices = {};
    for (const [uuid, price] of Object.entries(priceMap)) {
        prices[uuid] = price;
    }
    saveSkinsJSON();
}
...
export const getSkin = async (uuid, reloadData = true) => {
    if (reloadData && !skins) await fetchData([skins, prices]);

    let skin = skins && skins[uuid];
    if (!skin) return null;
...
export const getSkinFromSkinUuid = async (uuid, reloadData = true) => {
    ...
    let skin = skins && Object.values(skins).find(skin => skin.skinUuid === uuid);
    if (!skin) return null;
...
export const getWeapon = async (uuid) => {
    if (!weapons) await fetchData([skins]);

    return weapons ? weapons[uuid] || null : null;
}

export const getPrice = async (uuid) => {
    if (!prices) await fetchData([prices]);

    if (prices && prices[uuid]) return prices[uuid];
    ...
    if (bundle) {
        ...
        if (bundleItem?.price) return bundleItem.price;
    }

    // Fallback: estimate price from rarity tier if not yet cached from storefront
    const skin = skins && skins[uuid];
    const tierUuid = typeof skin?.rarity === "string" ? skin.rarity : skin?.rarity?.uuid;
    if (tierUuid) {
        const isMelee = skin.weapon === "2f59173c-4bed-b6c3-2191-dea9b58be9c7";
        if (tierUuid === "12683d76-48d7-84a3-4e09-6985794f0445") return isMelee ? 1750 : 875; // Select
        if (tierUuid === "0cebb8be-46d7-c12a-d306-e9907bfc5a25") return isMelee ? 2550 : 1275; // Deluxe
        if (tierUuid === "60bca009-4182-7998-dee7-b8a2558dc369") return isMelee ? 3550 : 1775; // Premium
        if (tierUuid === "e046854e-406c-37f4-6607-19a9ba8426fc") return isMelee ? 4350 : 2175; // Exclusive
        if (tierUuid === "411e4a55-4e59-7757-41f0-86a53f101bb5") return isMelee ? 4950 : 2475; // Ultra
    }
