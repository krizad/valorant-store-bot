    ...
    return new Promise((resolve, reject) => {
        const req = https.request(url, {
            ...
            headers: {
                ...
                "referer": "https://github.com/krizad/valorant-store-bot", // to help other APIs see where the traffic is coming from
                ...options.headers
            },
...
export { WeaponType, WeaponTypeUuid } from "./weaponTypes.js";
...
// }
// Default fallback version data in case of race condition or network delay
let riotVersionData = {
    manifestId: "C330A20409C5FDF2",
    branch: "release-09.04",
    version: "09.04.00.2747317",
    buildVersion: "21",
    engineVersion: "4.27.2.0",
    riotClientVersion: "release-09.04-shipping-21-2747317",
    riotClientBuild: "86.0.3.1523.3366",
    buildDate: "2024-05-13T00:00:00Z"
};
...
export const extractTokensFromUri = (uri) => {
    if(!uri || typeof uri !== "string") return [null, null];

    try {
        // Order-independent token regex matches
        const accessTokenMatch = uri.match(/access_token=([a-zA-Z0-9.\-_]+)/);
        const idTokenMatch = uri.match(/id_token=([a-zA-Z0-9.\-_]+)/);

        let accessToken = accessTokenMatch ? accessTokenMatch[1] : null;
        let idToken = idTokenMatch ? idTokenMatch[1] : null;

        if(accessToken && idToken) {
            return [accessToken, idToken];
        }

        // Fallback using URL query or hash parameters
        const fragmentOrQuery = uri.includes("#") ? uri.split("#")[1] : (uri.includes("?") ? uri.split("?")[1] : uri);
