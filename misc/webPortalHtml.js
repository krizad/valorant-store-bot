...
export function renderWebPortalHtml({ session, publicUrl = "", lang = "" }) {
    ...
    const replacements = {
        ...
        "{{TAB_EXTENSION}}": escapeHtml(t.tabExtension || "Chrome Extension (1-Click)"),
        ...
        "{{EXTENSION_TITLE}}": escapeHtml(t.extensionTitle || "⚡ 1-Click Browser Extension Helper"),
        "{{EXTENSION_HINT}}": escapeHtml(t.extensionHint || "Install extension to auto-sync SSID"),
        "{{EXTENSION_DOWNLOAD_BTN}}": escapeHtml(t.extensionDownloadBtn || "📥 Download Extension (.zip)"),
        "{{EXTENSION_INSTALL_GUIDE}}": escapeHtml(t.extensionInstallGuide || "1. Extract ZIP ➔ 2. chrome://extensions ➔ 3. Developer mode ➔ 4. Load unpacked"),
        "{{EXTENSION_DETECTED_NOTICE}}": escapeHtml(t.extensionDetectedNotice || "✓ Extension detected!"),
        "{{EXTENSION_SYNC_NOW_BTN}}": escapeHtml(t.extensionSyncNowBtn || "⚡ 1-Click Link with Extension"),
