...
import {ActionRowBuilder, ButtonBuilder, ButtonStyle, escapeMarkdown, EmbedBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, AttachmentBuilder} from "discord.js";
import {CanvasBannerService} from "../services/canvasBanner.js";
...
export const renderOffers = async (shop, interaction, valorantUser, VPemoji, otherId=null) => {
    ...
    if (config.useStoreBanner) {
        ...
        for(const uuid of shop.offers) {
            ...
            if (!skin) continue;
            const price = isDefaultSkin(skin) ? 0 : (skin.price || 0);
            skinsData.push({
        ...
        try {
            const bannerBuffer = await CanvasBannerService.generateStoreBanner(skinsData);
            if (bannerBuffer) {
...
export const skinCollectionSingleEmbed = async (interaction, id, user, {loadout, favorites}) => {
    ...
    const fields = [
        ...
        await createField(WeaponTypeUuid.Bandit),
        emptyField,
    ...
    const levels = await getSkinLevels(skinsUuid.filter(item => item?.uuid).map(item=>item.uuid), interaction);
    if(levels) components.unshift(levels);
...
export const skinCollectionPageEmbed = async (interaction, id, user, {loadout, favorites}, pageIndex=0) => {
    ...
    const pages = [
        ...
        [WeaponTypeUuid.Shorty, WeaponTypeUuid.Bucky, WeaponTypeUuid.Judge, WeaponTypeUuid.Bandit],
...
export const botInfoEmbed = (interaction, client, guildCount, userCount, registeredUserCount, ownerString, status) => {
    const fields = [
        ...
        },
        {
            ...
            value: "[ValorantStoreCheck](https://github.com/krizad/valorant-store-bot) by KriZad\n*(Based on [SkinPeek](https://github.com/giorgi-o/SkinPeek) by [Giorgio](https://github.com/giorgi-o))*",
            inline: true
        }
    ];
...
export const helpEmbed = (interaction) => {
    const isThai = interaction?.locale === "th";

    return {
        embeds: [{
            title: isThai ? "📖 คู่มือคำสั่งทั้งหมดของบอท (Commands Guide)" : "📖 Valorant Store Check — Commands Guide",
            description: isThai
                ? "รายการคำสั่ง Slash Commands ทั้งหมดที่ใช้งานได้ในบอท ตรวจสอบร้านค้า สกิน และจัดการบัญชีของคุณได้อย่างสะดวก"
                : "Complete list of available slash commands to check your Valorant daily store, alerts, and accounts.",
            color: VAL_COLOR_1,
            fields: [
                {
                    name: isThai ? "🛒 ร้านค้าและสกิน (Store & Cosmetics)" : "🛒 Store & Cosmetics",
                    value: isThai
                        ? "` /shop [user] ` — ดูร้านค้า 4 สกินประจำวันของคุณ (พร้อมแบนเนอร์รูป 2x2)\n" +
                          "` /accessoryshop ` — ดูร้านค้า Accessory Store ประจำสัปดาห์ (Kingdom Credits)\n" +
                          "` /bundles ` — ดูบันเดิลพิเศษที่กำลังวางจำหน่ายในปัจจุบัน\n" +
                          "` /bundle <name> ` — ตรวจสอบไอเทมและราคาของบันเดิลที่ระบุ\n" +
                          "` /nightmarket ` — ดูร้านค้าตลาดกลางคืน Night Market พร้อมส่วนลด\n" +
                          "` /collection [weapon] [user] ` — ดูสกินปืนทั้งหมดหรือสกินของอาวุธที่ระบุ"
                        : "` /shop [user] ` — Show your daily 4-skin store (renders 2x2 canvas banner)\n" +
                          "` /accessoryshop ` — Show current weekly Accessory Store offers (Kingdom Credits)\n" +
                          "` /bundles ` — Show currently featured weapon collection bundles\n" +
                          "` /bundle <name> ` — Inspect items and price of a specific bundle\n" +
                          "` /nightmarket ` — Show current Night Market discount offers\n" +
                          "` /collection [weapon] [user] ` — Inspect owned weapon skins",
                    inline: false
                },
                {
                    name: isThai ? "🔔 การแจ้งเตือนและสถิติ (Alerts & Stats)" : "🔔 Alerts & Stats",
                    value: isThai
                        ? "` /alert <skin> ` — ตั้งแจ้งเตือนเมื่อสกินที่ต้องการเข้าในร้านค้าประจำวัน\n" +
                          "` /alerts ` — ดูและจัดการรายการแจ้งเตือนสกินทั้งหมดของคุณ\n" +
                          "` /testalerts ` — ทดสอบการส่งแจ้งเตือนและสิทธิ์ของบอทในช่องนี้\n" +
                          "` /stats [skin] ` — ดูสถิติความถี่และอันดับที่สกินเคยสุ่มเข้ามาในร้านค้า"
                        : "` /alert <skin> ` — Set an alert for when a specific skin appears in your store\n" +
                          "` /alerts ` — View and manage your active skin alerts\n" +
                          "` /testalerts ` — Test alert delivery and permissions in this channel\n" +
                          "` /stats [skin] ` — View frequency statistics of skins appearing in stores",
                    inline: false
                },
                {
                    ...
                    value: isThai
                        ? "` /login [ssid] ` — เข้าสู่ระบบ (ใส่ SSID เพื่อเปิดแจ้งเตือนร้านค้า 07:00 น. ถาวร)\n" +
                          ...
                          "` /account <name> ` — สลับบัญชี Valorant ที่กำลังใช้งานในบอท\n" +
                          "` /accounts ` — แสดงรายชื่อบัญชี Valorant ทั้งหมดที่คุณผูกไว้\n" +
                        ...
                        : "` /login [ssid] ` — Log in securely (use SSID cookie for permanent 07:00 daily alerts)\n" +
                          ...
                          "` /account <name> ` — Switch active Valorant account\n" +
                          "` /accounts ` — List all linked Valorant accounts\n" +
                ...
                {
                    name: isThai ? "⚙️ การตั้งค่าและระบบ (Settings & System)" : "⚙️ Settings & System",
                    value: isThai
                        ? "` /settings view ` — ดูการตั้งค่าความเป็นส่วนตัวปัจจุบัน\n" +
                          "` /settings set ` — ปรับแต่งการตั้งค่า (เช่น ซ่อนชื่อ, อนุญาตให้เพื่อนส่องร้าน)\n" +
                          "` /valstatus ` — ตรวจสอบสถานะการทำงานของเซิร์ฟเวอร์ Valorant\n" +
                          "` /info ` — แสดงข้อมูล สถิติ และสถานะการทำงานของบอท\n" +
                          "` /help ` — แสดงหน้ารายการคำสั่งทั้งหมดนี้"
                        : "` /settings view ` — View your current privacy & bot settings\n" +
                          "` /settings set ` — Modify preferences (hide IGN, allow others to view shop)\n" +
                          "` /valstatus ` — Check status of Riot Valorant servers\n" +
                          "` /info ` — Show bot information, server count, and uptime\n" +
                          "` /help ` — Show this command help guide",
                    inline: false
                }
            ],
            footer: {
                text: isThai ? "Valorant Store Check • พิมพ์ / ตามด้วยชื่อคำสั่งเพื่อเริ่มใช้งาน" : "Valorant Store Check • Type / to start using commands"
            }
        }]
    };
};
